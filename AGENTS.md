...
- JSON: all emitted JSON field names are camelCase (`generatedAt`, `nodesInCycles`, `cutCandidates`, `extFanIn`, `fanIn`, `mutPorts`, ...); CLI table headers use the same names. The export graph format is called the *standard JSON export format* (page title/label; never "common JSON").
