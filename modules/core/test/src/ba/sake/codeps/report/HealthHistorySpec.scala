...
class HealthHistorySpec extends munit.FunSuite:
  ...
  private def snapshot(nodes: Int = 100, ratio: Option[Double] = Some(0.2)) =
    HealthSnapshot(
      ...
      structure = HealthStructure(nodes, 200),
  ...
  test("history comparison records when either scope changes") {
    val previous = entry()
    assertEquals(HealthHistory.decision(Some(previous), entry(packages = snapshot(nodes = 101), files = Some(snapshot())), 0.01, false), HealthRecordingDecision.NotSignificant)
