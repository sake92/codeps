...
object Filter:
  /**
    ...
    * Childless package nodes are pruned afterwards (`Prune.emptyPackages`), and
    * scoped calls retain only symbol references sourced by surviving files and
    * targeting surviving public declarations.
    */
  def apply(graph: DepsGraph, includes: Seq[String], excludes: Seq[String]): DepsGraph =
    ...
    val retained = Prune.emptyPackages(DepsGraph(universe, keptEdges, graph.symbolReferences, graph.declaredPublicSymbols))
    SymbolMetadataFilter(retained, retained.nodes, includes.nonEmpty || excludes.nonEmpty)
